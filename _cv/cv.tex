...
\cventry{2005}{Stage de fin d'études}{AREVA NP, département Développement}{}{}{%
  \begin{itemize}
...
\cventry{2004}{Stage de recherche}{Univ. de Californie, Santa Barbara (UCSB),
  dept. Génie Chimique}{}{}{%
  \begin{itemize}
...
\cvitem{Encadrement}{Suivi de quatre thèses avec divers partenaires : École
  Polytechnique de Montréal, Imperial College, Université Pierre et Marie Curie,
  CEA.}

\cvitem{Communication scientifique}{Articles et présentations dans des revues et conférences
  internationales, \begin{itemize}
...
\cvitem{Schémas avancés}{\vspace{-0.85em}\begin{itemize}
...
\cventry{2017}{Ecole d'été PRECIS {\footnotesize(Précision et REproductibilité en Calcul
  Informatique et Scientifique)}}{3h}{}{}{%
...
\cvitem{Wesley Ford\\(2016--)}{{\bfseries Vers des méthodes d'accélération
    stables et efficaces en contextes parallèles appliquées à l'équation du
    transport des neutrons}~:\begin{itemize}
  \item Directeur : Christophe Calvin (CEA, Maison de la Simulation).
  \end{itemize}\vspace{-1em}\strut}

\cvitem{Romain Picot\\(2015--2018)}{{\bfseries Vérification numérique de
    logiciels de calculs industriels}~:\begin{itemize}
  \item Directrice : Fabienne Jézéquel (Université Pierre et Marie Curie, Paris 6).
  \end{itemize}\vspace{-1em}\strut}

\cvitem{Rebecca Jeffers\\(2013--2016)}{{\bfseries\itshape Goal Based Coupled
    Adaptive Mesh Refinement (AMR) and angular adaptivity on Cartesian Meshes
    for Modelling Neutron Transport in PWR Reactor Cores}~:\begin{itemize}
  \item Directeur : Matthew D. Eaton (Imperial College London).
  \end{itemize}\vspace{-1em}\strut}

\cvitem{\hspace{-5em}Marc-André Lajoie\\(2011--2017)}{{\bfseries Développement
    d’un schéma de calcul prismatique généralisé parallèle en transport
    déterministe hétérogène 3-D}~:\begin{itemize}
  \item Co-directeur : Guy Marleau (École Polytechnique de Montréal).
  \end{itemize}\vspace{-1em}\strut}

\subsection{Encadrement de stagiaires}

\cvitem{Morgane Steins\\(2018)}{{\bfseries Amélioration de la convergence
    spatiale des éléments finis en neutronique.}
  \begin{itemize}
...
\cvitem{Mehdi Ouafi\\(2017)}{{\bfseries Conditionnement des problèmes
    d'optimisation de la production hydraulique}
  \begin{itemize}
...
\cvitem{Louis Denoix\\(2017)}{{\bfseries Stabilité numérique des algorithmes
    d'optimisation}.
  \begin{itemize}
...
\cvitem{Chou Xiaochen\\(2016)}{{\bfseries Gestion des erreurs numériques dans
...
\cvitem{Guy Augarde\\(2015)}{{\bfseries Exploration du potentiel du langage
    Julia pour le calcul scientifique.}
  \begin{itemize}
  \item Développement d'une bibliothèque d'algèbre linéaire par blocs
    hiérarchiques en langage Julia.
  \end{itemize}\vspace{-1em}\strut}

\cvitem{Salli Moustafa\\(2013)}{{\bfseries Vectorisation d'un algorithme de
    transport neutronique en géométries prismatiques.}
  \begin{itemize}
...
\cvitem{Ulrick Séverin\\(2012)}{{\bfseries Intégration fine du flux dans la
    plate-forme \textsc{Cocagne}}
  \begin{itemize}
